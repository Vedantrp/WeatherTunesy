...
function setErr(msg){ errorBox.textContent = msg; show(errorBox); setTimeout(()=>hide(errorBox), 5000); }
...
function updateAuthUI(){
  if (accessToken && user){
    hide(loginBtn);
    show(userInfo);
    userName.textContent = user.display_name || user.id || "Spotify user";
  } else {
    show(loginBtn);
    hide(userInfo);
  }
...
searchBtn.onclick = async () => {
  try{
    ...
    wText.textContent = w.condition;
    wTemp.textContent = Math.round(w.temp_c);
    show(weatherCard);
